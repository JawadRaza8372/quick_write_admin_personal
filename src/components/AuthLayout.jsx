...
import { loginFun } from "../helpers/endPoints";
...
const AuthLayout = () => {
	...
	const formSubmitFun = async (data) => {
		try {
			dispatch(startLoadingModal());
			await loginFun(data.email, data.password)
				.then(async (response) => {
					const { tokens, ...rest } = response?.user;
		...
		<div className="flex-1 grid grid-cols-1 lg:grid-cols-2 4xl:grid-cols-[minmax(0,_1030px)_minmax(0,_1fr)]">
			{width >= 1024 && (
				<div className="flex flex-col justify-center items-center gap-[25px] lg:p-[60px]">
					...
					/>
					<img
						className="max-w-[780px] h-[320px] object-contain"
						src={media}
						alt="media"
					/>
				</div>
			...
			<div className="bg-widget flex items-center justify-center w-full py-10 px-4 lg:p-[60px]">
				<Spring
					...
					<form
						className="mt-5"
						onSubmit={handleSubmit(formSubmitFun)}>
